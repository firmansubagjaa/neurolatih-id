...
const SequenceGame: React.FC<SequenceGameProps> = ({ difficulty, onEndGame, onBack, isQuickMode = false, isPracticeMode = false }) => {
  ...
  const fetchNextQuestion = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) setCurrentQuestion(q);
    ...
    if (viewState === 'GAME' && !isPracticeMode && gameActive && introFinished && !showTutorial && !showQuitModal && timeLeft > 0) {
      timerIntervalRef.current = setInterval(() => {
        setTimeLeft((prev) => {
          ...
          if (newVal <= 0) { handleFinish(); return 0; }
          return newVal;
        });
      }, 100);
  ...
  const handleAnswer = (index: number) => {
    // Hardened guard clause against race conditions
    if (selectedOption !== null || !gameActive || loading) return;
    ...
    setTimeout(() => {
      if (isMountedRef.current && gameActive) {
         if (!isPracticeMode && timeLeft <= 0) handleFinish();
        ...
        <div className="w-full max-w-2xl mx-auto space-y-4 animate-fade-in h-[calc(100vh-100px)] flex flex-col justify-center">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 {[
                     ...
                     { id: 'COMPLEX', name: "COMPLEX", sub: "Abstrak", icon: <Layers />, desc: "Fibonacci, Prima." },
                     { id: 'CHAOS', name: "CHAOS", sub: "Multilayer", icon: <Shuffle />, desc: "Pola selang-seling." },
                 ].map((m) => (
                     <Card key={m.id} onClick={() => startGame(m.id as LogicMode)} className="cursor-pointer hover:border-retro-green hover:-translate-y-1 transition-all group">
                         <div className="flex items-center gap-3 mb-2">
                             <div className="text-retro-green">{m.icon}</div>
                             <div><h3 className="font-pixel text-sm">{m.name}</h3><span className="text-[10px] bg-slate-800 px-1 text-slate-300">{m.sub}</span></div>
                         </div>
  ...
  return (
    <div className="w-full max-w-3xl mx-auto relative h-[calc(100vh-100px)] flex flex-col">
      ...
      <TutorialOverlay isOpen={showTutorial} onClose={() => setShowTutorial(false)} title={`Cara Bermain: ${logicMode}`} content={["Analisis deret angka.", "Temukan pola logikanya.", "Pilih angka selanjutnya."]} icon={<Network className="w-6 h-6" />} />
      <QuitModal isOpen={showQuitModal} onConfirm={() => { setShowQuitModal(false); onBack(); }} onCancel={() => setShowQuitModal(false)} />

      <div className="flex justify-between items-center mb-4 shrink-0">
        <div className="flex gap-2">
          <Button variant="ghost" onClick={() => { playSound('click'); isPracticeMode ? handleFinish() : setShowQuitModal(true); }} className="!px-2 text-xs">&larr; Mode</Button>
          <Button variant="ghost" onClick={() => setShowTutorial(true)} className="!px-2 text-neuro-400 text-xs"><HelpCircle className="w-4 h-4" /></Button>
        </div>
        <div className="flex gap-2">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        {loading ? (
          <Card className="flex-1 flex flex-col items-center justify-center animate-pulse border-none bg-transparent">
            <NeuralLoader message="GENERATING..." />
          </Card>
        ) : currentQuestion ? (
          <div className="flex-1 flex flex-col animate-fade-in-up">
            {/* Question Area */}
            <div className="flex-1 flex flex-col items-center justify-center text-center bg-slate-800/50 rounded-xl border border-white/5 p-4 mb-4 relative">
               <div className="absolute top-2 left-2 p-2 bg-emerald-900/30 rounded-full"><Network className="w-6 h-6 text-emerald-400" /></div>
               <h3 className="text-2xl md:text-4xl font-mono font-bold text-white leading-relaxed break-words w-full">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-6 shrink-0 mb-4">
              {currentQuestion.options.map((option, idx) => {
                let baseClass = "p-4 md:p-8 rounded-xl border-2 transition-all flex items-center justify-center gap-2 text-xl md:text-3xl font-bold relative overflow-hidden ";
                let stateClass = "border-white/10 bg-slate-800 text-slate-300 "; 

                if (selectedOption !== null) {
                  if (idx === currentQuestion.correctAnswerIndex) {
                    stateClass = "border-emerald-500 bg-emerald-500/20 text-emerald-100 glow-success ";
                  } else if (idx === selectedOption) {
                    stateClass = "border-red-500 bg-red-500/20 text-red-100 ";
                  } else {
                    stateClass = "border-white/5 opacity-30 ";
                  }
                } else {
                    stateClass += "active:scale-95 active:bg-slate-700 active:text-white md:hover:border-emerald-400 md:hover:bg-slate-700 md:hover:scale-105 ";
                }

                return (
                  <button key={idx} onClick={() => handleAnswer(idx)} disabled={selectedOption !== null} className={baseClass + stateClass}>
                    {option}
                    {selectedOption !== null && idx === currentQuestion.correctAnswerIndex && <CheckCircle className="w-5 h-5" />}
                    {selectedOption !== null && idx === selectedOption && idx !== currentQuestion.correctAnswerIndex && <XCircle className="w-5 h-5 text-red-400" />}
                  </button>
                );
              })}
            </div>
            ...
            {showExplanation && (
              <div className="animate-fade-in shrink-0 min-h-[60px]">
                 <div className={`p-3 rounded-lg text-sm md:text-base text-center border-2 ${selectedOption === currentQuestion.correctAnswerIndex ? 'bg-emerald-900/20 border-emerald-500/30 text-emerald-200' : 'bg-red-900/20 border-red-500/30 text-red-200'}`}>
