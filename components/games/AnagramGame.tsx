  ...
  return (
    <div className="w-full max-w-lg mx-auto relative">
      ...
      <TutorialOverlay 
        ...
        content={["Susun ulang huruf acak.", "Bentuk satu kata baku Bahasa Indonesia.", "Gunakan keyboard di layar."]}
         ...
         {loading ? <NeuralLoader message="ENCRYPTING..." /> : question && (
             <>
                <div className="w-full mb-6 text-center">
                    <p className="text-xs text-slate-500 mb-2 uppercase tracking-widest">HINT: {question.hint}</p>
                    <div className="text-3xl md:text-5xl font-mono text-slate-400 tracking-[0.5em] mb-4 break-words">{question.scrambled}</div>
                    
                    {/* Input Display */}
                    <div className="flex justify-center gap-2 mb-8 flex-wrap min-h-[50px]">
                        {Array.from({ length: question.original.length }).map((_, i) => (
                            <div key={i} className={`w-8 h-10 md:w-10 md:h-12 border-b-4 flex items-center justify-center text-xl font-bold ${userInput[i] ? 'border-retro-green text-retro-green' : 'border-slate-700'}`}>
                                {userInput[i] || ''}
                            </div>
                        ))}
                    </div>
                ...
                {/* Keyboard */}
                <div className="w-full grid grid-cols-6 gap-1 md:gap-2 mb-4">
                    {question.scrambled.split('').sort().map((char, i) => (
                        ...
                        <button key={i} onClick={() => handleType(char)} className="p-2 bg-slate-800 border border-slate-600 rounded active:bg-slate-700 font-mono text-lg hover:border-retro-green text-white">
                            {char}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex w-full gap-2">
                    <Button variant="secondary" onClick={handleBackspace} className="flex-1"><Delete className="w-4 h-4" /></Button>
                    <Button onClick={checkAnswer} className="flex-[3]"><Key className="w-4 h-4 mr-2" /> SUBMIT</Button>
                </div>
             </>
         )}
      </Card>
